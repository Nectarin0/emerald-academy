...
Глава 4: [Хранилище аккаунта](#хранилище-аккаунта) ([Раздел `/storage/`](#раздел-storage))
...
❗ Важно понимать, что доступ к этим разделам происходит через `AuthAccount`, который передается в секцию `prepare` при совершении транзакции, поэтому необходимо обращать внимание на то, что вы подписываете.
...
1) Конструкция `<T>` между именем функции и списком параметров указывает тип данных, с которым работает функция. Это требуется поскольку мы не знаем, что за данные хранятся в `StoragePath`.
2) `StoragePath` представляет из себя просто путь `/storage/любой/путь`.
3) `load` и `borrow` возвращают опционал, так как мы не знаем наверняка, хранится ли что-то по переданному пути.
